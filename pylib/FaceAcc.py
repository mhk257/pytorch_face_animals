...
########## rmse ##########
def per_image_rmse(pred, ann, norm_type):
    ...
    for i in range(N):
        ...
        if L == 9:
            ...
            height, width = np.max(pts_gt, axis=0) - np.min(pts_gt, axis=0)
            face_size = np.sqrt(width ** 2 + height ** 2)
        ...
        if norm_type == 'face_size':
            rmse[i] = np.sum(np.linalg.norm(pts_pred-pts_gt, axis=1))/(face_size*L)           
        else:
