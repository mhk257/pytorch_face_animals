...
def show_sample(inputs, target):
    ...
    for n in range(num_sample):
        ...
        for p in range(num_joints):
            ...
            colored_heatmap = color_heatmap(target[n,p,:,:])*0.5
            colored_heatmap = np.transpose(colored_heatmap, (2, 0, 1))
           
            tgt = inp*0.5 + torch.from_numpy(colored_heatmap).float()
...
def sample_with_heatmap(inp, out, num_rows=2, parts_to_show=None):
    ...
    #print('Num rows {} Num cols {} Size {}'.format(num_rows,num_cols,size))
...
def sample_with_heatmap2(inp, out, num_rows=2, parts_to_show=None):
    inp = to_numpy(inp * 255)
    out = to_numpy(out)
    ...
    for i, part in enumerate(parts_to_show):
        ...
        dec_dim_H_sel = out_resized.astype(float)

        h_heatmap_max = dec_dim_H_sel.max()
        h_heatmap_min = dec_dim_H_sel.min()
        h_heatmap_show = (dec_dim_H_sel - h_heatmap_min) / (h_heatmap_max - h_heatmap_min)
        h_heatmap_show = h_heatmap_show * 255
        h_heatmap_show = cv2.applyColorMap(h_heatmap_show.astype(np.uint8), cv2.COLORMAP_HSV)
        im_H = Image.fromarray(h_heatmap_show)
        #im_H = np.asarray(h_heatmap_show).astype(np.uint8)
        #print(im_H.shape)
        ...
        #out_img = inp_small.copy() * .3
        # color_hm = color_heatmap(out_resized)
        #out_img = out_img + (im_H * 0.7)
        
        col_offset = (i % num_cols + num_rows) * size
        row_offset = (i // num_cols) * size
        full_img[row_offset:row_offset + size, col_offset:col_offset + size] = im_H
    
    
    return full_img

def batch_with_heatmap(inputs, outputs, mean=torch.Tensor([0.5, 0.5, 0.5]).cuda(), num_rows=4, parts_to_show=None):
    batch_img = []
...
def batch_with_heatmap2(inputs, outputs, mean=torch.Tensor([0.5, 0.5, 0.5]).cuda(), num_rows=4, parts_to_show=None):
    batch_img = []
    for n in range(min(inputs.size(0), 4)):
        inp = inputs[n] + mean.view(3, 1, 1).expand_as(inputs[n])
        batch_img.append(
            sample_with_heatmap2(inp.clamp(0, 1), outputs[n], num_rows=num_rows, parts_to_show=parts_to_show)
        )
    return np.concatenate(batch_img)
